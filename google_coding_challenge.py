...
blocks = [{req: (True if random() <= exists_p else False) for req in reqs} for _ in range(5_000)]
if print_blocks:
...
def timer(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        print(f'Function took {time_taken} microseconds')
        return result
    return wrapper
...
def solution_2(reqs, blocks):
    ...
    while i < max_i:
        for j in range(len(blocks) - i):
            ...
            if sum(any(block[req] for block in block_group) for req in reqs) == len(reqs):
                complete_block_groups[j] = block_group
        if complete_block_groups and max_i == len(blocks):
            num_rows, num_cols = i, len(reqs)
            max_min_total_dist = (num_rows - 1) * math.floor(num_cols / 2)
            max_i = max_min_total_dist + 1
        i += 1
    if i == 1:
        ...
        for k, (index_offset, complete_block_group) in enumerate(complete_block_groups.items()):
            best_block_index, min_total_dist = solution_1.__wrapped__(reqs, complete_block_group)
